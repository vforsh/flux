...
import { buildFillBody, type OutputFormat } from "../../models/builders.ts";
...
export function registerFill(program: Command): void {
  program
    .command("fill")
    .description("Inpaint using FLUX Fill (mask or alpha channel)")
    .argument("[prompt]", "prompt text, or '-' to read from stdin")
    .requiredOption("--image <path|url|base64>", "input image")
    .option("--mask <path|url|base64>", "mask image (optional; alpha channel also supported)")
    ...
    .action(async (promptArg: string | undefined, options: any, cmd: Command) => {
      ...
      const maskBase64 = options.mask ? await toBase64(String(options.mask)) : undefined;

      let modelPath = "flux-pro-1.0-fill";
      ...
      if (options.body) {
        ...
        if (maskBase64 && body.mask == null) body.mask = maskBase64;
      } else {
        const built = buildFillBody({
          prompt,
          imageBase64,
          maskBase64,
          steps: options.steps,
